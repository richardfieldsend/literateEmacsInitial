...
- Dell Latitude E5570 laptop (ziggy-latitude2)
...
#+begin_src emacs-lisp
      ...
      (if (string=(system-name) "ziggy-latitude2")
	  (setq initial-scratch-message
...
#+begin_src emacs-lisp
  ...
  (setq kept-new-versions 6)			       ; no. of new files to keep
...
#+begin_src emacs-lisp
  (use-package autumn-light-theme
    :ensure t
    :init (load-theme autumn-light-3
theme t))
#+end_src
...
#+end_src
You do need to run the following command to make it work (within
Emacs):
M-x all-the-icons-install-fonts

As you would expect, this installs the fonts required to make the
pretty icons work! The full github repository for this is here:
...
The most useful shortcuts for this are:

C-c C-e : Insert environment. When first starting the document it
works to create the appropriate framework for the document including
begin and end.

C-c C-s : Insert a section. The section types appropriate for the
document type are listed. This is followed by the RefTeX function to
add a label.

C-c C-m : Insert a macro. Helm filtering available.
